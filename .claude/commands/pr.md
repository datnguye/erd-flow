...
Use the **pr** skill to open a pull request for the current branch.
...
`$ARGUMENTS` verbatim if a name is given). Then run the **gates** (`tsc
--noEmit` + `vitest run`) and fold the real results into the checklist,
